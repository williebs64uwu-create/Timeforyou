...
function renderProfile(profile) {
    ...
    if (profile.avatar_url) {
        ...
        document.getElementById('pAvatar').src = `https://ui-avatars.com/api/?name=${encodeURIComponent(profile.name)}&background=3B82F6&color=fff`;
    }
...
async function checkAvailability(dateStr) {
    ...
    const { data, error } = await window.supabaseClient
        ...
        .or('status.eq.confirmed,status.eq.pending') // Block pending too? Yes, safer.
    ...
    // Extract times
    return data.map(b => b.start_datetime.split('T')[1].substring(0, 5));
}
