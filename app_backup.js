...
let classes = [];
let exams = [];
let bookings = []; // New: Citas agendadas
...
function startReminderChecker() {
    log('🚀 Iniciando sistema de recordatorios MEJORADO');
...
window.onload = function () {
    log('🚀 Iniciando TickTick Ultra PRO...');
...
Revisa tu bandeja de entrada (y spam) para entrar.`, 'Enlace Mágico Enviado', 'success');
    } catch (error) {
...
function loadData() {
    ...
    log(`📊 Datos cargados: ${tasks.length} tareas, ${habits.length} hábitos, ${classes.length} clases`);
}
...
function showLevelUpModal(newLevel) {
    ...
    modal.innerHTML = `
        <div class="modal-content" style="text-align: center; background: linear-gradient(135deg, #1a1a1a 0%, #2d2d2d 100%); border: 2px solid var(--primary);">
            ...
            <h1 style="color: var(--primary); font-size: 32px; margin-bottom: 10px;">¡LEVEL UP!</h1>
            <p style="font-size: 18px; margin-bottom: 20px;">Has alcanzado el <strong>Nivel ${newLevel}</strong></p>
            <div style="font-size: 14px; color: var(--text-secondary); margin-bottom: 30px;">¡Sigue así, máquina! 🚀</div>
            <button class="btn btn-primary" onclick="this.parentElement.parentElement.remove()">¡VAMOS!</button>
        </div>
    `;
    document.body.appendChild(modal);
        ...
        if (dayTasks.length === 0 && dayBookings.length === 0) {
            ...
            if (dayBookings.length > 0) {
                dayBookings.forEach(booking => {
                    html += `
                        <div class="task-card" style="margin-bottom: 6px; padding: 8px; border-left: 3px solid var(--accent); background: var(--bg-tertiary);">
...
function openInputModal(title, fields, onSubmit, prefillData = {}) {
...
function renderBookingRequests(content) {
    ...
    const pendingBookings = bookings.filter(b => b.status === 'pending').sort((a, b) => new Date(a.start_datetime) - new Date(b.start_datetime));

    let html = `
        <div style="margin-bottom: 25px; display: flex; justify-content: space-between; align-items: center;">
            <div>
                <h2 style="margin:0;">Solicitudes de Citas</h2>
                <p style="color:var(--text-secondary); margin:5px 0 0 0; font-size:14px;">Gestiona tus reuniones pendientes.</p>
            </div>
            <div style="background: var(--bg-tertiary); padding: 5px 15px; border-radius: 20px; font-size: 13px; color: var(--text-secondary);">
    ...
    if (pendingBookings.length === 0) {
        html += `
            <div style="text-align: center; padding: 60px 20px; color: var(--text-secondary);">
                <div style="font-size: 48px; margin-bottom: 15px; opacity: 0.5;"><i class="ri-calendar-check-line"></i></div>
                <h3 style="margin-bottom: 10px;">Estás al día</h3>
                <p>No tienes solicitudes de citas pendientes.</p>
            </div>
        `;
    } else {
        html += `<div style="display: grid; gap: 15px; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr));">`;

        pendingBookings.forEach(booking => {
            const dateObj = new Date(booking.start_datetime);
            const dateStr = dateObj.toLocaleDateString('es-ES', { weekday: 'long', day: 'numeric', month: 'long' });
            const timeStr = dateObj.toLocaleTimeString('es-ES', { hour: '2-digit', minute: '2-digit' });

            html += `
                <div class="task-card" style="border-left: 4px solid var(--accent); background: var(--bg-secondary);">
                    <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 15px;">
                        <div style="display: flex; gap: 12px; align-items: center;">
                            <div style="width: 40px; height: 40px; background: var(--bg-tertiary); border-radius: 50%; display: flex; align-items: center; justify-content: center; color: var(--accent); font-size: 20px;">
                                <i class="ri-user-line"></i>
                            </div>
                            <div>
                                <div style="font-weight: 700; font-size: 16px;">${booking.guest_name}</div>
                                <div style="font-size: 12px; color: var(--text-secondary);">${booking.guest_email}</div>
                            </div>
                        </div>
                    ...
                    <div style="background: var(--bg-primary); padding: 12px; border-radius: 8px; margin-bottom: 15px;">
                        <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 8px; color: var(--text-primary); font-weight: 500;">
                            <i class="ri-calendar-event-line" style="color: var(--text-tertiary);"></i>
                            ${dateStr}
                        </div>
                        <div style="display: flex; align-items: center; gap: 10px; color: var(--text-primary); font-weight: 500;">
                            <i class="ri-time-line" style="color: var(--text-tertiary);"></i>
                            ${timeStr} (60 min)
                        </div>
                        ${booking.notes ? `
                            <div style="margin-top: 10px; padding-top: 10px; border-top: 1px solid var(--border); font-size: 13px; color: var(--text-secondary); font-style: italic;">
                                "${booking.notes}"
                            </div>
                        ` : ''}
                    </div>
                    ...
                    <div style="display: flex; gap: 10px;">
                        <button class="btn btn-primary" style="flex: 1;" onclick="handleBookingAction('${booking.id}', 'confirmed')">
                            <i class="ri-check-line"></i> Aceptar
                        </button>
                        <button class="btn btn-secondary" style="flex: 1;" onclick="handleBookingAction('${booking.id}', 'cancelled')">
...
async function handleBookingAction(bookingId, newStatus) {
    if (!confirm(newStatus === 'confirmed' ? '¿Aceptar esta cita y añadirla a tu agenda?' : '¿Rechazar esta solicitud?')) return;

    try {
        console.log('🧠 Supabase Client:', window.supabaseClient ? 'Initialized' : 'MISSING! Check config.');
        ...
        window.customAlert = function (message, title = 'Notificación', type = 'info') {
            return new Promise((resolve) => {
                const modal = document.getElementById('customAlertModal');
                if (!modal) {
                    console.warn('Custom alert modal not found, using native alert');
                    alert(message);
                    resolve();
                    return;
                }
                ...
                iconParent.className = 'modal-icon ' + type;
        ...
        window.closeCustomConfirm = function (result) {
            const modal = document.getElementById('customConfirmModal');
            modal.classList.remove('show');
            if (customConfirmCallback) customConfirmCallback(result);
        };

        // Custom Prompt (replaces window.prompt)
        window.customPrompt = function (message, defaultValue = '', title = 'Ingresa información') {
            return new Promise((resolve) => {
                ...
                input.onkeypress = (e) => {
                    if (e.key === 'Enter') closeCustomPrompt('submit');
                };
            });
        };

        window.closeCustomPrompt = function (action) {
            const modal = document.getElementById('customPromptModal');
            const input = document.getElementById('promptInput');
            const value = action === 'submit' ? input.value : null;

            modal.classList.remove('show');
            if (customPromptCallback) customPromptCallback(value);
        };
...
// ========================================
        ...
        // update local state pending re-sync
        const index = bookings.findIndex(b => b.id === bookingId);
        if (index !== -1) bookings[index].status = newStatus;

        if (newStatus === 'confirmed') {
            showToast('✅ Cita confirmada y agendada', 'success');
        } else {
